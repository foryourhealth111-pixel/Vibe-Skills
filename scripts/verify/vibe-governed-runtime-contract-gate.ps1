...
Add-Assertion -Results ([ref]$results) -Condition ((@($specialistRecommendationIds).Count -ge 1) -or $noSpecialistNeeded) -Message 'runtime smoke freezes bounded specialist recommendations or explicit no-specialist decision'
Add-Assertion -Results ([ref]$results) -Condition ((@($specialistRecommendationIds) -contains 'systematic-debugging') -or $noSpecialistNeeded) -Message 'runtime smoke preserves systematic-debugging or explicit no-specialist decision'
...
Add-Assertion -Results ([ref]$results) -Condition (([int]$executionManifest.specialist_accounting.recommendation_count -ge 1) -or $noSpecialistNeeded) -Message 'runtime smoke execution manifest carries specialist accounting or explicit no-specialist decision'
Add-Assertion -Results ([ref]$results) -Condition (([int]$executionManifest.plan_shadow.specialist_dispatch_unit_count -ge 1) -or $noSpecialistNeeded) -Message 'runtime smoke plan shadow counts specialist dispatch units or explicit no-specialist decision'
