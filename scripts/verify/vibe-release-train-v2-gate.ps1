param(
    ...
    function Write-GateArtifacts {
        ...
        $jsonPath = Join-Path $dir 'vibe-release-train-v2-gate.json'
        $mdPath = Join-Path $dir 'vibe-release-train-v2-gate.md'
        Write-VgoUtf8NoBomText -Path $jsonPath -Content ($Artifact | ConvertTo-Json -Depth 100)

        $lines = @(
            '# VCO Release Train v2 Gate',
            '',
    ...
    [pscustomobject]@{ path = 'docs/releases/README.md'; keywords = @(
    'Wave64-82',
    ...
    $artifact = [pscustomobject]@{
        gate = 'vibe-release-train-v2-gate'
        repo_root = $repoRoot
