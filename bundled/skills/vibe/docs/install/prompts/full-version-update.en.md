...
5. Keep `claude-code` and `cursor` described as supported install-and-use paths, `windsurf` as a supported install-and-use path with runtime-adapter integration, `openclaw` with the `preview` / `runtime-core-preview` / `runtime-core` wording plus `OPENCLAW_HOME` or `~/.vibeskills/targets/openclaw` and the attach / copy / bundle paths, and `opencode` as a preview-adapter path with default target root `OPENCODE_HOME` or `~/.vibeskills/targets/opencode`, plus direct install/check wording without ownership of the real `opencode.json`.
